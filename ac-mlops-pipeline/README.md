...
│   │                              work happens here.
│   │
│   ├── evaluation              <- evaluate  model step before deploying a model.
│   │
│   ├── monitoring              <- Optional - Model monitoring, feature monitoring, etc.
│   │
│   ├── inference              <- Batch inference code that will run as part of scheduled workflow.
│   │
│   │
...
│       │
│       ├── clv-lifetimevalue-bucket-classifier-DE.yml <- E2E Workflow for lifitime value specific to the country "DE"
│       │
│       ├── clv-repurchase-classifier-DE.yml      <- E2E Workflow for reourchase classifier specific to the country "DE"
│       │
│       ├── *.yml                                 <- Other yml resources can be treated as an example template
│   
│
...
the [project README](./ac_mlops_pipeline/README.md).
...
After that, follow the [ML pull request guide](docs/ml-pull-request.md)
 and [ML resource config guide](ac_mlops_pipeline/resources/README.md)  to propose, test, and deploy changes to production ML code (e.g. update model parameters)
or pipeline resources (e.g. use a larger instance type for model training) via pull request.

| Role                          | Goal                                                                         | Docs                                                                                                                                                                |
|-------------------------------|------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Data Scientist                | Get started writing ML code for a brand new project                          | [project README](./ac_mlops_pipeline/README.md) |
...
| Data Scientist                | Modify production model ML resources, e.g. model training or inference jobs  | [ML resource config guide](ac_mlops_pipeline/resources/README.md)  |
...
need to run it again for project `ac-mlops-pipeline`.
